...
public class ChercherPat extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String nom= request.getParameter("txtnompat");
		...
		if(session.getAttribute("login")==null){
			...
			try {
			String req = "select * from patient where id_pat='"+id+"'and nom_pat='"+nom+"'";
			 ResultSet rs = st.executeQuery(req);
			
			while(rs.next()){
				
				out.print("le patient est : "+rs.getString(2)+" "+rs.getString(3)+" "+rs.getString(4)+" "+rs.getString(5)+" "+rs.getString(6));
